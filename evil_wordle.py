...
class Keyboard:
    ...
    def __str__(self):
        ...
        """
        keyboard_str = ""
        row_indents = [0, 1, 3]  # Indentation for each row
        for indent, row in zip(row_indents, self.rows):
            keyboard_str += " " * indent
            keyboard_str += " ".join(color_word(self.colors[char], char) for char in row) + "\n"
        return keyboard_str.strip()
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i in range(NUM_LETTERS):
        if feedback[i] == NOT_IN_WORD_COLOR and guessed_word_list[i] in secret_word_list:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    word_family_objects = fast_sort(word_family_objects)
    hardest_family = word_family_objects[0]
